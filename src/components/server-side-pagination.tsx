...
export const ServerSidePagination = ({ meta, links, url }: ServerSidePaginationProps) => {
    return (
        meta.has_pages && (
            <div className='flex w-full items-center justify-center sm:gap-x-8'>
                <div className='flex items-center justify-end gap-x-1.5'>
                    {meta.current_page !== 1 ? (
                        <Link
                            href={`${url}?page=${getPageNumberFromUrl(links.first)}`}
                    ...
                    {links.prev ? (
                        <Link
                            href={`${url}?page=${getPageNumberFromUrl(links.prev)}`}
                            className={cn(buttonVariants({ size: 'icon', variant: 'outline' }))}>
                    ...
                    {links.next ? (
                        <Link
                            href={`${url}?page=${getPageNumberFromUrl(links.next)}`}
                            className={cn(buttonVariants({ size: 'icon', variant: 'outline' }))}>
                    ...
                    {meta.to !== meta.total ? (
                        <Link
                            href={`${url}?page=${getPageNumberFromUrl(links.last)}`}
                            className={cn(buttonVariants({ size: 'icon', variant: 'outline' }))}>
